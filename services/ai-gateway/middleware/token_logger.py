...
async def log_usage(db_pool: asyncpg.Pool, record: UsageRecord) -> None:
    ...
    """
    # RLS (app_user): ai.ai_usage_logs carries WITH CHECK, so this INSERT needs the
    # tenant GUC on the same transaction — see db/tenant_scope.py.
    async with tenant_scoped(db_pool, record.tenant_id) as conn:
        await conn.execute(
            """
