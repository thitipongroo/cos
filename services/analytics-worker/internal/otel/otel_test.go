...
func TestSamplingRatio(t *testing.T) {
	tests := []struct {
		...
		{"default when unset", "", 0.01},
		{"parses a decimal", "0.25", 0.25},
		{"parses 1", "1", 1},
		{"falls back on garbage", "not-a-number", 0.01},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			t.Setenv("OTEL_SAMPLING_RATIO", tc.env)
			if got := samplingRatio(); got != tc.want {
				t.Errorf("samplingRatio() = %v, want %v", got, tc.want)
			}
		})
	}
}
