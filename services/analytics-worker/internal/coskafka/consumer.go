...
func (c *Consumer) Run(ctx context.Context, brokers []string, group string, topicRegex string) error {
	client, err := kgo.NewClient(
		...
		// and re-reading is safe: the carbon insert is idempotent by construction — a unique index
		// on consumption_id in Postgres and ReplacingMergeTree on carbon_record_id in ClickHouse.
