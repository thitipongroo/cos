...
| PDPA-48 | Subject rights for people WITHOUT a platform account                         | Routed to the tenant, which is the controller (ADR-090 §3), and the tenant now has the mechanism: `POST /api/v1/subject-requests` · `GET …` · `GET …/:id/matches` · `POST …/:id/erase` · `PATCH …/:id/close` (TENANT_ADMIN only; `platform.subject_requests` + RLS). Search is bound to a request row and audited with the match count; erasure anonymises in place. `PARTIAL`, not `DONE`: the API is the whole of it — there is no admin SCREEN yet, so a tenant without an integrator drives this from an API client, and the legal-hold archive in ADR-090 §5 has no store behind it (a row under `legal_hold` is anonymised with no pre-image kept) | `PARTIAL` | 2026-08-16 |
