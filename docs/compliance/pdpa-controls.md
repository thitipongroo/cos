...
| PDPA-48 | Subject rights for people WITHOUT a platform account                         | Routed to the tenant, which is the controller (ADR-090 §3). `PARTIAL`, not `DONE`: the routing decision and the record are in place, the `TENANT_ADMIN` search / export / erase tooling (ADR-090 §4) is not yet built — until it ships a tenant has no mechanism here beyond a manual database request        | `PARTIAL` | 2026-08-16 |
