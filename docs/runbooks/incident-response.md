...
> here, a paging drill. PagerDuty and the status page are both required *before* Stage 2 go-live
...
> SLAs. The labels were shifted by one, so an alert called "P1" meant *complete outage* to QM-17 and
> *partial outage* to this runbook. QM-17 wins; the labels below are its labels.
