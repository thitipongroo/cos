...
| PDPA-02 | §37(2)   | Prevent unauthorised disclosure to third parties  | RLS `rls_tenant_isolation` on every domain table; `app_user` never granted BYPASSRLS (ADR-031); `isolation-tests` job in `.github/workflows/ci.yml:408` runs `jest --testPathPatterns='tenant-isolation'`, which matches both `backend/test/security/01-tenant-isolation-and-audit.integration.spec.ts` (RLS under `app_user`) and `.../03-tenant-isolation.integration.spec.ts` (the API layer) | `DONE`    | 2026-08-03 |
