...
whole repository: exactly three production `Worker.create` call sites exist —
`backend/src/modules/procurement/workflows/worker.ts`,
`services/file-service/src/extraction/worker.ts` and `services/file-service/src/cleanup/worker.ts`.
Every other occurrence is a test. **None of the three is launched by any script, Dockerfile, Compose
service, CI step or Helm chart**, and none appears in `32-implementation-specifications` §32.2's
deployable table. The file-service Dockerfile runs `main.js` and nothing else; the Helm chart sets no
command or args.
...
| #     | Question                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Status                     |
| ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------- |
| OQ-32 | **No Temporal worker is launched anywhere in the platform, and one workflow has no worker at all.** Three production `Worker.create` sites exist — procurement, file cleanup, file zip-extraction — and none is started by a script, Dockerfile, Compose service, CI step or Helm chart, nor listed in `32-implementation-specifications` §32.2. Separately, `data-export.service.ts:319` starts `dataExportWorkflow` in production and **no worker file registers it**, so the PDPA data-export path has no executor even in principle. Meanwhile the Temporal **server** is in `docker-compose.yml`, so every one of these workflows is accepted and recorded as running. This supersedes the phase-local framing in [OQ-25](README.md#open-questions-register). Verified statically across the repository. | Open — needs a PO decision |
