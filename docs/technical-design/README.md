...
| OQ-50 | `finance.cashflow_risk.detected.v1` + `finance.budget.exceeded.v1`                  | Both have committed `.avsc` schemas, `topic-catalog` entries, `infrastructure/kafka/topics.yaml` rows and spec references (15, 16, 32), and **no producer anywhere in the repository** — verified 2026-08-23 across all sources excluding build output. Either they are future-dated and the spec should say so, or a producer was dropped. | Phase 7                    |
