...
**As of 2026-08-23: 53 entries, all 53 closed.** That is a statement about this register, not about
...
| OQ-51 | `KeycloakJwtStrategy` vs `platform.tenants.keycloak_realm` (§7.6)                   | **Closed 2026-08-23 — validation now resolves the issuer per tenant, and binds it.** The pattern is the standard resource-server one (Spring Security packages it as `JwtIssuerAuthenticationManagerResolver`): resolve the token's `iss`, check it against a trusted-issuer allowlist, fetch that issuer's JWKS. The allowlist is not configuration here — it is `SELECT keycloak_realm FROM platform.tenants WHERE is_active = true`, cached 60s, so a newly provisioned ENTERPRISE realm works without redeploying the backend and a deactivated tenant's realm stops being trusted. `issuer` is no longer passed to passport-jwt: a static option cannot express a list that changes, and it is unnecessary because an issuer outside the allowlist never yields a signing key. **The second half is the binding:** `validate()` now selects `t.keycloak_realm` and rejects unless it equals the realm in `payload.iss`. Signature and audience prove a token is genuine and meant for this API; neither says which TENANT it may act as, and `tenant_id` is a Keycloak user attribute — so whoever can create users in any trusted realm would otherwise pick it. That is the shape of the Keycloak security-policy defect that verified signatures but never tied `iss` to the configured realm, letting a token from any realm through; accepting multiple issuers WITHOUT the binding is how it gets built by accident. JWKS is still fetched from `KEYCLOAK_URL` + the realm, never from the `iss` host — split-horizon deploys keep working and a token cannot point the backend at a server of its choosing. Rejection reuses the existing message, so realm membership is not probeable. Keycloak Organizations (GA in 26, and enabled in our 26.6.4) was considered and not taken: §7.6 gives dedicated realms only to ENTERPRISE, which matches the industry guidance for realm-per-tenant, and those are the CIS/FIPS customers who need realm-level isolation of admins, themes and flows. 10 new tests; both guarantees falsified. | Phase 2, Phase 25          |
| OQ-52 | `infrastructure/monitoring/isolation-probe/configmap.yaml` vs its script          | **Closed 2026-08-23 — found and fixed in the same pass.** The CronJob runs `node /scripts/isolation-probe.js`, mounted from `configmap.yaml`, and that ConfigMap held a PLACEHOLDER: eight lines printing "regenerate from isolation-probe.js" then `process.exit(1)`, against 161 lines of real script beside it. Its own comment said to rebuild it via kustomize or helm and nothing ever did. The consequence is the worst shape a monitoring failure takes: the probe is the synthetic cross-tenant check (§30.6) whose `tenant_isolation_check_result` feeds `TenantIsolationBreach`, a **P0** — and that alert fires on a value of **0**, which an absent series never produces. A probe that exits 1 publishes nothing, so the control meant to notice a cross-tenant leak would have been silently dead, indistinguishable from "no breach". Surfaced while wiring the `cos-isolation-probe` ArgoCD Application, which would otherwise have deployed the placeholder. The real script is now embedded, and `scripts/ci/check-isolation-probe-configmap.mjs` fails the build on drift in either direction (falsified by editing the script without regenerating). Readiness AUTO-29 would also have caught it once deployed: it requires the CronJob to have a `lastSuccessfulTime`, and a placeholder that exits 1 never earns one. | Phase 16                   |
| OQ-53 | `infrastructure/kubernetes/pgbouncer/` vs QM-18                                     | **Closed 2026-08-23 — `auth_query` instead of a file it could never mount (PO decision).** `pgbouncer.ini` set `auth_type = scram-sha-256` with `auth_file = /etc/pgbouncer/userlist.txt`, and `/etc/pgbouncer` is mounted read-only from a ConfigMap whose only key is `pgbouncer.ini` — so the auth file was never going to exist and PgBouncer could authenticate nobody, while QM-18 puts it in front of every service and `db-failover.md` step 4 restarts it after a failover. A userlist holds password hashes and is cluster-specific, so committing one was never an option either. `auth_query` removes the file: PgBouncer connects as one role and asks PostgreSQL for the hash of whoever is connecting, so adding an application role needs no config change and no redeploy — which matters where `app_user` and `cos` are already two roles and every dedicated ENTERPRISE database adds more. **It goes through a SECURITY DEFINER function, not straight at `pg_shadow`:** reading `pg_shadow` requires superuser, and a superuser pooler would undo the whole reason `app_user` exists (owners and superusers bypass RLS — migration 20260623000001). Verified on the live database: `pgbouncer_auth` reads a SCRAM hash through the function, gets 0 rows rather than an error for an unknown user, and is refused both `pg_shadow` directly ("permission denied for view pg_shadow") and any application schema ("permission denied for schema platform"). Migration `20260823000001_pgbouncer_auth_query` with its rollback; the role ships NOLOGIN so no password enters the migration history. **Still ops:** that password, and the `pgbouncer-secrets` Secret, which exists nowhere in the repository. | Phase 17 |
