...
It reaches exactly three tables: `crm.contacts` (name, email, phone), `crm.leads` (contact_name) and
`procurement.vendors`. It does **not** reach `platform.users` or `workforce.workers`, both of which
hold personal data — see [OQ-48](#open-questions-register).
...
| OQ-48 | `11-database-schema` §11.4 PII erasure vs `SubjectRequestService.erase`          | **The erasure that exists reaches three tables and misses the two holding the platform's own people.** §11.4 says PII-bearing entities carry `pii_erased_at` and that erasure stamps it: **no table in the database has that column.** What is built instead is anonymisation in place — `POST /subject-requests/:id/erase`, irreversible by design because a hard delete would break `crm.contacts.lead_id` out of a seven-year Thai accounting chain (ADR-090 §5) — and it updates `crm.contacts`, `crm.leads` and `procurement.vendors` only. `platform.users` (`display_name`, `email`, `phone_number`) and `workforce.workers` (`full_name`, `contact_phone`) are untouched, so a PDPA §33 request from a site worker or a platform user cannot be satisfied. Whether those two are anonymised, deactivated, or out of scope because the tenant is not their data controller is a product and legal decision, not a code one — a `platform.users` row also anchors audit logs, memberships and FKs that must survive. | Phase 2, Phase 16, Phase 22 |
