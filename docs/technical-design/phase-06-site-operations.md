...
| #     | Question                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Status                  |
| ----- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| OQ-28 | **`LAST_WRITE_WINS` orders by a device clock, with no stated bound.** `client_submitted_at` comes from the handset, and the command says so. Nothing in the spec or the code caps how far it may diverge from server time, so a phone whose clock is a day fast wins every merge until it is corrected — and on shared field handsets, clock drift is not exotic. `server_received_at` exists and is trustworthy but is not the ordering key. Should skew be bounded, and what happens when it is exceeded? | Open — `UNSPECIFIED`    |
| OQ-29 | **`ep/file-service.stub.ts` throws and nothing calls it.** Its header says "Implemented in Phase 9 when File Service is built" — Phase 9 is built, and photo linkage went a different way (files carry `entity_id` back to the issue or report). The stub is dead code whose comment describes an integration that was never taken. Delete it, or record why the API-call design was dropped.                                                                                                               | Open — cleanup decision |
