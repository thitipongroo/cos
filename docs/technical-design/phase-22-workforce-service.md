...
anonymisation in place (`POST /subject-requests/:id/erase`). It updated `crm.contacts`, `crm.leads`
and `procurement.vendors` only, so a PDPA §33 request from a site worker could not be satisfied
against this table at all. **Fixed 2026-08-23** ([OQ-48](README.md#open-questions-register)):
`workforce.workers` is in both the search and the anonymisation, and so is `platform.users` — which
matters here because a worker with an account is reached through it, and because erasing the worker
row while leaving the account fully named would not be an erasure. See
...
| `workforce.checkin.created.v1`                    | ✅ present | all 6 fields since OQ-36; `method` is null — no capture path exists |
...
| #     | Question                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | Status                                                                              |
| ----- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------- |
| OQ-48 | **Closed for this phase 2026-08-23 — worker PII is erasable now.** §11.4's erasure table has listed an `Employee` entity with exactly `full_name` and `contact_phone` since it was written. There is no `employees` table: that is `workforce.workers`, the only table in the database carrying both columns — and neither `SubjectRequestRepository.findMatches` nor its `anonymise` touched it, so a site worker's PDPA §33 request against the tenant that employs them returned nothing and erased nothing. Both now cover it, matched by `contact_phone` directly or by email through `user_id` (the row has no email column; `uq_workers_user_id` keeps the join single-valued). `full_name = '[ERASED]'` because the column is NOT NULL, and `employee_code` / `trade_type` / `is_active` are deliberately untouched — a rights request is not a resignation, and flipping `is_active` would silently rewrite site rosters. Verified on the live database as `app_user` under RLS. The `platform.users` half — the account row and the Keycloak account behind it — was closed the same day; see the register. | Closed 2026-08-23                                                                   |
| OQ-36 | **Closed 2026-08-23 — and it was worse than recorded: the event had never reached Kafka.** The entry described 3 of 6 fields and a misnamed one. It also could not be Avro-encoded — `invalid "string": undefined` — so since ADR-094 every check-in has been a poison row in `platform.outbox_events`, retried ten times and retired, visible only to someone querying `attempts >= 10`. Two further corrections: `attendance_logs` HAS `log_id` (the `checkin_id` §32.4 wants) and `RecordAttendanceDto` DOES carry `latitude`/`longitude` — both were available all along. Only `method` truly could not be captured, so the enum became nullable, accepted as v2 under BACKWARD_TRANSITIVE, rather than being guessed at. Both payload shapes verified against a live registry; `workforce.checkout.created.v1` checked too and matches.                                                                                                                                                                                                                                                                          | Closed 2026-08-23                                                                   |
| OQ-37 | **The module README advertises a `BiometricCheckIn` stub that does not exist.** `backend/src/modules/workforce/README.md` says "Biometric / QR check-in available via EP-DOMAIN-008" and lists "EP-DOMAIN-008 `BiometricCheckIn` stub" among the module's extension points. `EP-DOMAIN-008` appears nowhere else in the repository — no stub file, no interface, no other reference. Not implementing it is correct (the command defers it); the README claiming otherwise is the drift, and it is the kind of claim that has twice been mistaken for a built control on this project.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Closed 2026-08-22 — resolution in the [register](README.md#open-questions-register) |
