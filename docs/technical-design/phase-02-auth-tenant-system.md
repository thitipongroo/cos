...
| OQ-48 | **`platform.users` is not reached by PDPA erasure, and §11.4 never listed it.** `POST /subject-requests/:id/erase` anonymises in place across `crm.contacts`, `crm.leads`, `procurement.vendors` and — since 2026-08-23 — `workforce.workers`. `platform.users` holds `display_name`, `email` and `phone_number` and is in none of them, so a platform user's own §33 request cannot be satisfied against the account record. It is not a simple omission to patch: the row anchors `platform.audit_logs.actor_id`, `tenant_memberships`, `consents`, `export_requests`, `trusted_devices` and more, all of which must outlive erasure, and `keycloak_user_id` points at an identity in a system this endpoint does not touch. §11.4 also specified a `pii_erased_at` column and a per-entity `pii.erased` audit entry; neither exists, and a per-REQUEST audit record already does. | Open — needs a PO decision |
