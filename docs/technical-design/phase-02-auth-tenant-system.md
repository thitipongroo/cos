...
| OQ-14 | Unified login lets any non-privileged role use either path, but nothing provisions a user for **both**: `POST /api/v1/users` takes a phone number or an email, not both, and §11.1 gives Path A accounts an empty-string email. Adding a second identifier to an existing account is unspecified.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | Open — needs a spec        |
| OQ-15 | `11-database-schema` §11.4 says **every** record carries `created_by` and `deleted_at` and that all records soft-delete. §11.1's own `platform.*` tables list neither — and `platform.audit_logs` must not be soft-deletable at all, since QM-4 makes it append-only. Whether §11.4 governs the `platform` schema or only domain tables is not stated.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Open — spec self-conflict  |
| OQ-17 | **QM-7 specifies a control the OTP login path does not have.** QM-7 requires "account lockout after 5 consecutive failures for 15 minutes" on authentication endpoints. Path A has per-code attempt capping (3), a 60 s resend cooldown, a 10/day per-phone budget and a 10/min per-IP throttle — but **no account lockout**. The only lockout in the codebase (`MFA_LOCKOUT_TTL_SECONDS = 900`) is in the **deprecated** custom TOTP module that ADR-067/074 record as wired into no login flow. Recorded in `docs/security/sms-otp-restricted-authenticator.md` § 3.3 as a gap, not a control.                                                                                                                                                                                                                                                                                                          | Open — QM-7 gap            |
