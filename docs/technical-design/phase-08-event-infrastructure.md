...
| OQ-6 | §32.4's "Required Canonical Names — Pending Spec Addition" table lists 27 legacy schema files to migrate before Phase 8. No legacy-named file remains on disk, but only **16 of the 27** canonical names exist; 11 have no file at all. Separately, `finance.variance.alert.v1.avsc` is present under its **legacy** name with a `.v1` suffix, where §32.4 requires `finance.budget.variance_detected.v1`. Whether the 11 were dropped, renamed differently, or are still owed is not stated.                                      | Open — needs a PO decision |
