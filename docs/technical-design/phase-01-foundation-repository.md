...
C4 Context and Container views: [`architecture/README.md`](../architecture/README.md). Level 3 is the
Core / Domain / Intelligence decomposition in `03-system-design` §3.2, with concrete wiring in
[`architecture/service-interaction.md`](../architecture/service-interaction.md). No diagram is drawn
...
| ID    | Question                                                                                                                                                                                                                                                                                                 | Status                 |
| ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------- |
| OQ-1  | `03-system-design` §3.2 names 21 logical services; `backend/src/modules/` holds 23 modules. Four §3.2 services (Workflow, Document, Quality Control, Asset Management, Forecasting) have no module of that name, and eight modules are not named in §3.2.                                                | Open — see README OQ-1 |
| OQ-1a | `@cos/schemas` and `@cos/ui-logic` exist on disk but are named in no phase `Generate:` list I have read. Their establishing decision was not located.                                                                                                                                                    | Open — needs a source  |
| OQ-5  | The root `pnpm-workspace.yaml` comment says `apps/mobile` installs "with `node-linker=hoisted` (`apps/mobile/.npmrc`)" via `pnpm install --ignore-workspace`. Both `apps/mobile/.npmrc` and `00_master` state the opposite — pnpm 10/11 reads `nodeLinker` from `pnpm-workspace.yaml`, **not** `.npmrc`. | Open — comment defect  |
