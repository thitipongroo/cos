...
| **Tenant-admin review queue (platform schema)**         | ❌ **absent**   | no table, no endpoint, no event — OQ-38                           |
| Serwist config with runtime caching                     | ✅ present      | `withSerwist` in `next.config.mjs`                                |
| `createSerwistRoute` with `useNativeEsbuild: false`     | ⚠️ **reversed** | set to `process.platform === 'win32'` — OQ-39                     |
