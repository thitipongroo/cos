...
| Temporal worker setup and registration            | ⚠️ **code only** | `workflows/worker.ts` exists and self-starts; **no process launches it** — OQ-25             |
...
| #     | Question                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Status                     |
| ----- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------- |
| OQ-25 | **Nothing starts the procurement Temporal worker, and the whole procurement state machine runs inside it.** The service only signals; activities own the status writes and the events. `worker.ts` is a standalone entrypoint with no `package.json` script, no Compose service, no Dockerfile, no CI step, no Helm chart, and no row in `32-implementation-specifications` §32.2's deployable table. The workflow unit tests pass because `TestWorkflowEnvironment` starts its own worker. **Failure scenario:** in any deployed environment, `POST /procurement/rfqs/:id/publish` returns 200 and the RFQ stays `DRAFT` forever; no `rfq.status_changed` event is emitted; the deadline timer never fires. Verified statically, not observed on a running deployment. | Open — needs a PO decision |
| OQ-26 | **Vendor scoring has no defined inputs.** `vendor-scoring.ts` states it itself: the adapter combines criterion values, but "how each criterion VALUE is derived from data (esp. `quality` and `price`) is NOT defined in the spec and is escalated to the product owner". `on_time_delivery` is derivable from `deliveries` vs `purchase_orders.delivery_date`; the other two are not derivable from anything this phase stores. `GET /procurement/vendors/:vendorId/score` is nonetheless a live endpoint.                                                                                                                                                                                                                                                             | Open — `UNSPECIFIED`       |
| OQ-27 | **`delivery_items` is load-bearing for PO fulfilment but appears in no data specification.** The Phase 5 command's `deliveries` entity has no line items, and `11-database-schema` does not carry the table — the only description of it anywhere is `17-offline-mobile-sync` §17.4, which explains its _semantics_ in passing while making an offline-sync point: "`delivery_items` are the quantities the platform sums to decide whether a PO line is fulfilled, so a double-applied replay would not merely duplicate a record, it would close a purchase order on goods that arrived once." A table that can close a purchase order deserves a definition in §11 and a row in the Phase 5 entity list, not a mention inside a sync rule.                           | Open — specification gap   |
