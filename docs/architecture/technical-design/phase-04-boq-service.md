...
| Category hierarchy deeper than two levels     | Copied intact since 2026-08-22 — the copy maps ids rather than matching `category_code` (§ 14 OQ-24) |
...
**A gap in the suite, not in the code alone.** No test placed items in a child category and asserted
the resulting version total, and no test copied a version whose categories were three levels deep.
Both OQ-23 and OQ-24 below were invisible to the suite as it stood — which is why they survived to be
found by reading. Both now carry regression tests proven to fail against the code they replaced.
...
| OQ-24 | **Closed 2026-08-22 — the copy is now recursive, and it was worse than reported.** The original entry described the depth loss: `copyVersionContents` inserted root categories, then in a second statement inserted children by joining the new version's categories on `category_code`, and that second `INSERT … SELECT` cannot see rows it is itself inserting, so a depth-3 category found no new parent and was dropped with its items. Reproducing it against PostgreSQL exposed a second defect on the same join: nothing makes `(version_id, category_code)` unique — there is no constraint in the DDL and `addCategory` does not check — so duplicate codes fanned the join out. A five-category source totalling **500.0000** copied to **4 categories and 600.0000**: a priced document that came out worth 20% more than the version it was copied from. `copyVersionContents` is now ONE statement that maps old category ids to new ones up front, so parents are matched by identity rather than by code and depth is irrelevant. After the fix: 5 / 5 / 500.0000 with depth 4 intact, verified as `app_user` under FORCE ROW LEVEL SECURITY and invisible to a second tenant. | Closed 2026-08-22             |
