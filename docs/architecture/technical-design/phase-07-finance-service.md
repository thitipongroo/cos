...
| OQ-50 | **Closed 2026-08-23 — both producers were built.** `finance.budget.exceeded.v1` and `finance.cashflow_risk.detected.v1` were the only 2 of 62 committed `.avsc` schemas with no producer anywhere — while still getting a provisioned topic, a generated type, an `EVENT_AVSC_MAP` entry and a slot in a readiness gate that refuses to pass until they are registered. Indistinguishable from a live event at every layer. `budget.exceeded` turned out to be blocked on attribution, not on the missing per-category budget column: `budget_lines` carries `boq_category_id`, and `cost_transactions.budget_line_id` existed from the first finance migration with **nothing ever writing it**, so every per-category figure read zero — including the task-completion gate in `TasksService`, which blocks at ratio ≥ 1.0 and so had never fired once. `procurement.po.created.v1` now carries `boq_item_id` per line (nullable with an Avro default, BACKWARD_TRANSITIVE), Finance resolves `boq_items.category_id` → `budget_lines.boq_category_id`, and a PO spanning several categories stays unattributed rather than charging one of them the whole total. `cashflow_risk` needed a product decision, taken the same day: grade by HOW SOON `cumulative_net` first goes negative — never → no event; weeks 9–13 LOW, 5–8 MEDIUM, 2–4 HIGH, 0–1 CRITICAL — swept daily by `CashflowRiskService`, which calls the same `buildForecast` the pull endpoint uses so an alert cannot disagree with the screen. `scripts/ci/check-event-producers.mjs` fails any schema without a producer unless it is listed declared-only WITH a reason, and fails a listed one that has since grown a producer — which is how each of these two transitions got noticed before a human did. | Closed 2026-08-23 |
