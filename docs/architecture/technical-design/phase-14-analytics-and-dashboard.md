...
| OQ-47 | **Closed 2026-08-23 — four independent faults, each enough on its own to keep every target table empty.** (1) `kafka_topic_list` named bare events (`construction.project.created`); real topics are `{tenant_id}.{event_type}.v1`. ClickHouse 26.3 accepts a `^`-anchored regex — the claim that it could not was mine and it was wrong. (2) The payload Tuples declared 2–3 of 6–8 fields. A partial Tuple is not a projection: the reader mis-reads the record and `kafka_skip_broken_messages = 100` throws the result away silently — 5 messages consumed, 0 exceptions, 0 rows. (3) `toDate(occurred_at)` failed on EVERY event, whatever the topic was called: `occurred_at` comes from `toISOString()` and ClickHouse stops at the `Z` — `syntax error at position 23`, the same failure `services/analytics-worker/internal/carbon/consumer.go` documents solving in Go and this file never did. (4) `toUUID()` throws on a malformed id, and an exception while pushing to a view aborts the whole block — one stale test record was discarding every good record batched with it. All four fixed and verified end to end on a clean volume: 8 tables assigned and reading, and the three daily tables carrying the exact figures published. A CI gate pairs each table against its `.avsc`. | Closed 2026-08-23 |
