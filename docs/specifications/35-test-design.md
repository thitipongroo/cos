...
| `TC-P14-INT-001` | Kafka → ClickHouse → API flow | INT | ClickHouse + Kafka containers | 1. Publish domain events; 2. Let the Kafka engine tables ingest; 3. Call the dashboard API | The dashboard reflects the published events | Ph14 Generate "Integration tests" | `GAP` — verification performed; the deliverable does not work. `backend/test/analytics-pipeline.integration.spec.ts` runs the real Kafka → ClickHouse → API path against the committed DDL and shows **no event can ever reach the dashboard**, for two independent reasons: the shipped `payload` Tuple cannot decode the shipped Avro schema (§35.13 ESC-39) and the application publishes to a topic the engine tables never subscribe to (§35.13 ESC-38). The spec pins the broken behaviour so it cannot go quiet again |
...
| `ESC-41` | Architecture collision (resolved) | **Both branches implemented the Phase 8 outbox, differently.** Ours wrote the event inside the business transaction (`OutboxPublisher.write(tx, …)` threaded through every repository); theirs added `EventOutboxService` + `OutboxPollerService` under ADR-094, durable but explicitly *not* transactional | Their own header says it: *"Durable, not transactional… the classic pattern gets that by threading `tx` from the repository into the publish call — this codebase's repositories own their transactions internally, so that is a refactor of every write path"*. That refactor is exactly what ESC-13 had already done. Their service also already exposed `write(tx, …)` for the case | Two pollers against one `platform.outbox_events` would both claim rows. Choosing either side alone would have discarded a real guarantee — atomicity on one hand, the retry/backoff protocol on the other | **RESOLVED 2026-08-24 — both kept, one poller.** Their `EventsModule` (service + poller) is the single relay, because their poller implements the `attempts` / `last_error` / `next_attempt_at` claim protocol that migration `20260819000003_outbox_delivery_state` added columns for; ours knew nothing about them. Our transactional call sites stay as they are. Our duplicate `shared/outbox/outbox-poller.service.ts` was deleted. Where an event has no row to anchor to (contract signatures, platform-scope events, Temporal compensation), it goes through their durable `publish()` — each such site now says why in a comment |
