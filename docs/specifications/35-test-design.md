...
| `ESC-34` | Type/runtime mismatch — **OPEN, product owner decision** | `ExecutiveDashboardRow.atRisk` is declared `boolean`, but it is produced by a ClickHouse `if(...)`, which returns `UInt8`. The value on the wire is `0` / `1`, never `false` / `true` | ESC-33 asserted `toBe(false)` and received `0`. The declared type is at `backend/src/modules/analytics/analytics.service.ts:24` | **No behaviour is wrong today** — the only consumer, `apps/web/src/app/(app)/alerts/page.tsx:27`, filters on truthiness, and `0` / `1` behave correctly there. The risk is future: `r.atRisk === true` is false for an at-risk project, and strict schema validation on the response would reject the payload | **OPEN.** Two fixes, and the choice is a product one because one of them changes an API response shape: **(a)** coerce in the service (`atRisk: Boolean(row.atRisk)`) so the runtime matches the declared contract — correct-by-design, but the JSON changes from `0` / `1` to `false` / `true`; **(b)** change the declared type to `0 | 1` so it stops overstating — zero wire change, but the API keeps a C-style boolean. Not decided unilaterally. The test asserts the **actual** runtime shape and cites this row, so it stays honest under either outcome |
