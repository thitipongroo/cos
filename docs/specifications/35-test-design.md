...
| `TC-P06-INT-002` | Site reports and issues are indexed for full-text search | INT | OpenSearch available | 1. Create a report and an issue; 2. Search by free text | Both are retrievable | Ph6 Generate "OpenSearch indexing" | `PLANNED` — OpenSearch is globally stubbed in the integration harness (§30.4) |
...
| `TC-P07-INT-002` | Finance never queries procurement tables directly | INT | App running | 1. Trace the queries issued while consuming procurement events | Only finance-schema tables are touched; cross-module data arrives via Kafka | Ph7 Constraints; master §4; Rule 3 | `PLANNED` — no automated assertion |
...
| `TC-P09-ISO-001` | A Tenant A signed URL cannot read a Tenant B object | ISO | Two tenant buckets | 1. Generate a signed URL as Tenant A; 2. Use it against a Tenant B object key | 403 Forbidden | §30.6 S3 row | `PLANNED` — not located in the isolation suite |
...
| `TC-P10-UNIT-018` | React Native never uses IndexedDB, and the web app never uses expo-sqlite | UNIT | Both codebases | 1. Search `apps/mobile` for IndexedDB usage; 2. Search `apps/web` for `expo-sqlite` | Neither appears | Ph10 platform rules; file 02 Offline Sync constraints | `PLANNED` — no automated assertion |
...
| ID | Title | Level | Pre-condition | Steps | Expected result | Spec ref | Status |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `TC-P11-UNIT-001` | No caller invokes the OpenAI SDK directly | UNIT | AI service sources | 1. Search for direct SDK imports outside the provider implementation | Every LLM call goes through `LLMProvider` | Rule "Never call OpenAI SDK directly"; Ph11 | `PLANNED` — no automated assertion |
...
| `TC-P12-UNIT-015` | Token budget is capped at 4,000 input / 1,000 output | UNIT | Oversized context | 1. Assemble a context exceeding 4,000 tokens | The context is truncated or the request refused; output is capped at 1,000 tokens | Ph12 Generate "Token budget enforcement" | `PLANNED` — enforcement assertion not located |
...
| `TC-P13-INT-002` | Uniqueness constraints exist on `{label}.{id}` + `tenant_id` | INT | Migrated Neo4j | 1. List constraints; 2. Attempt a duplicate insert | Constraints exist for every label; duplicates are rejected | Ph13 Generate "Neo4j schema constraints" | `PLANNED` — constraint assertion not located |
...
| `TC-P13-ISO-001` | Graph traversal cannot cross tenants | ISO | Two tenant subgraphs | 1. Traverse from a Tenant A node toward Tenant B | Zero results | §30.6 Neo4j row | `PLANNED` — not located in the isolation suite |
...
| `TC-P15-UNIT-007` | `console.log` is not used anywhere in application code | UNIT | Source tree | 1. Search for `console.log` outside tests and tooling | No occurrence — `@cos/logger` is used | QM-8; Rule "Never use console.log" | `PLANNED` — no automated assertion |
...
| `TC-P16-INT-001` | `audit_logs` rejects UPDATE and DELETE from the application role | INT | Migrated DB, `app_user` | 1. Attempt `UPDATE`; 2. Attempt `DELETE` | Both denied by policy | Ph16 Immutable logging | `PLANNED` — not located |
...
| `TC-P16-ISO-002` | Cross-tenant Kafka messages are rejected | ISO | Shared consumer | 1. Deliver a message whose `tenant_id` header does not match the topic tenant | Message rejected; the DLQ is not populated with the cross-tenant message | §30.6 Kafka row | `PLANNED` — not located in the isolation suite |
...
| `TC-P17-INT-001` | The application connects through PgBouncer in every environment | INT | Deployed stack | 1. Resolve `DATABASE_URL` in each environment | Always the PgBouncer service; never the database host on 5432 | QM-18 | `PLANNED` — same assertion as `TC-P01-INT-001` |
...
| `TC-P21-INT-001` | Utilisation records land in the TimescaleDB hypertable | INT | Testcontainers on the `timescale/timescaledb` image | 1. `POST /api/v1/equipment/:id/utilization`; 2. Query `equipment_telemetry.equipment_utilization` | The row is stored in the hypertable partitioned by `recorded_at` | Ph21 TimescaleDB Tables; §30.4 harness | `PLANNED` — not located |
| `TC-P21-ISO-001` | Equipment APIs are tenant-isolated | ISO | Two tenant fixtures | 1. Read Tenant B's equipment with Tenant A's JWT | 403 / zero rows | §30.6 | `PLANNED` — not located in the isolation suite |
...
| `TC-P22-UNIT-009` | `verifyCheckIn` accepts `FINGERPRINT` and returns a boolean | UNIT | Biometric adapter injected via DI | 1. Call `verifyCheckIn(workerId, projectId, 'FINGERPRINT')` | Resolves to a boolean | §13.5 interface | `UNSPECIFIED` — §13.5 defines only the signature; no acceptance criterion, threshold, error path or timeout is specified (ESC-04) |
| `TC-P22-UNIT-010` | `verifyCheckIn` accepts `FACE_ID` and returns a boolean | UNIT | Adapter injected | 1. Call with `'FACE_ID'` | Resolves to a boolean | §13.5 interface | `UNSPECIFIED` — as `TC-P22-UNIT-009` (ESC-04) |
| `TC-P22-UNIT-011` | `verifyCheckIn` accepts `IRIS` and returns a boolean | UNIT | Adapter injected | 1. Call with `'IRIS'` | Resolves to a boolean | §13.5 interface | `UNSPECIFIED` — as `TC-P22-UNIT-009` (ESC-04) |
...
| `TC-P23-AI-003` | `SafetyVisionModel` meets its evaluation threshold | AI | 10,000+ labelled photos | 1. Train; 2. Evaluate | `UNSPECIFIED — escalate`: no primary metric, secondary metric or pass threshold is defined in §30.11, §22.6 or `00_master` Phase 23 | §35.13 ESC-02 | `UNSPECIFIED` |
| `TC-P23-AI-004` | `GraphMLModel` meets its evaluation threshold | AI | 6+ months of graph data | 1. Train; 2. Evaluate | `UNSPECIFIED — escalate`: no metric or threshold defined in any spec | §35.13 ESC-02 | `UNSPECIFIED` |
...
| --- | --- | --- |
| `IMPLEMENTED` | 406 | A test file, script or configuration was observed on disk and is cited in the case row |
| `PLANNED` | 73 | The case is designed and derived from the spec, but no corresponding test asset was located |
| `UNSPECIFIED` | 7 | The spec does not define the expected result — blocked in §35.13 |
| `GAP` | 3 | Verification was performed and the deliverable under test is missing or non-conforming |
| **Total** | **489** | |
...
| Backend integration | `backend/test/` — 11 specs incl. `tenant-isolation.integration.spec.ts` |
...
| Case | Finding | Status |
| --- | --- | --- |
| `TC-P08-UNIT-010` | The Outbox Pattern is specified and unit-tested but **not wired** — no production code calls `OutboxPublisher.write()` and no bootstrap starts `OutboxPoller`, so event delivery is not atomic with the database transaction | **OPEN** — ESC-13. The QM-4 defect inside the outbox (unqualified `INSERT INTO outbox_events`) is fixed with a regression test; the write-path wiring across 19 call sites is scoped in §35.13 and tracked as its own change |
...
**Remaining call sites (18 of 19).** Convert with the pattern above:

| Service | Events still published directly |
| --- | --- |
| `project` | `updated`, `status_changed`, `archived` (3) |
| `boq` | 4 |
| `procurement` | 5 |
| `tenant` | 4 |
| `user` | 2 |
| `equipment`, `workforce`, `finance`, `site-ops`, `notification` | hold a `KafkaProducer` directly rather than a `publishEvent` helper |

Until each lands, that service's `publishEvent` catch block **loses** the event on Kafka failure —
it is logged, not retried. The comment in `project.service.ts` that claimed otherwise was corrected.
There is no double-publish risk: a converted path writes to the outbox and never publishes directly.

Still outstanding for the §30.4 critical test: an integration test proving **DB rollback → no event
emitted** (requires Testcontainers, not a unit mock).
