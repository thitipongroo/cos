...
| `TC-P08-UNIT-010` | The outbox writes in the same transaction as the business entity | UNIT | Outbox publisher | 1. Write an entity and its outbox row in one transaction; 2. Force a rollback | On rollback neither the entity nor the outbox row persists — and no event is emitted | Ph8 Outbox Pattern; §30.4 critical test | `IMPLEMENTED` (partial) — the in-transaction write is unit-covered in `packages/@cos/kafka/src/__tests__/outbox.spec.ts` and, since 2026-08-23, at **every** converted call site: each repository spec asserts the outbox INSERT went through the same tx handle as the business write, and each service spec asserts the envelope handed to that write. The **rollback → no event** half still needs a Testcontainers integration test — §35.13 ESC-13 |
...
| --- | --- | --- |
| `IMPLEMENTED` | 411 | A test file, script or configuration was observed on disk and is cited in the case row |
| `PLANNED` | 73 | The case is designed and derived from the spec, but no corresponding test asset was located |
| `UNSPECIFIED` | 5 | The spec does not define the expected result — blocked in §35.13 |
