...
| --- | --- | --- |
| `IMPLEMENTED` | 410 | A test file, script or configuration was observed on disk and is cited in the case row |
| `PLANNED` | 78 | The case is designed and derived from the spec, but no corresponding test asset was located |
| `RETIRED` | 1 | `TC-P21-UNIT-001` — superseded; kept for traceability |
...
Two corrections fell out: `IMPLEMENTED` was overstated by one (`TC-P21-UNIT-001` is `RETIRED`, a
status the table did not list at all), and the last five `UNSPECIFIED` cases — `TC-P22-UNIT-009/010/011`
...
| `ESC-24` | Process gap | The QM-1 Python coverage gate was specified but never wired. QM-1 states it verbatim — "`pytest --cov` with `--cov-fail-under=99` for lines" — yet all four AI services ran bare `pytest`, none had `pytest-cov` in `requirements-dev.txt`, and none had a coverage config | Read `context.md:145`; `grep -i pytest-cov services/*/requirements*.txt` returned nothing; the `ai-service-tests` step was `run: pytest` | The gate QM-1 mandates was absent, and the shortfall it exists to expose stayed invisible | **RESOLVED 2026-08-23** (product owner: wire it now, per QM-1) — `pytest-cov==7.1.0` pinned in each service, a `.coveragerc` added per service, and the CI step is now `pytest --cov=. --cov-config=.coveragerc --cov-report=term-missing --cov-fail-under=99`. **Measured first (python:3.12-slim, source only): ai-embedding-worker 73%, ai-ocr-pipeline 46%, ai-gateway 45%, ai-transcription-pipeline 35% — every one below the 99% floor, so this gate fails until the missing tests are written.** That is the point: QM-1's requirement is now asserted rather than assumed. The `.coveragerc` omits `tests/` because measuring a suite against itself inflates the result sharply here — with tests included the same four report 87/73/64/52% |
