...
| `keycloak_realm`   | VARCHAR(100)                                | NOT NULL · UNIQUE except `construction-os` | STARTER/PROFESSIONAL share `construction-os` (§7.6); every other realm one tenant — partial unique index `tenants_keycloak_realm_dedicated_key` since 20260914000001 |
...
| `position`         | VARCHAR(255) | NULL                         | Job title, free text, for display — the drawer profile block's POSITION line (§32.7). Shaped after `department`: an account is created before anyone knows the title. Not a role and not an authorisation input. Migration `20260908000001`, ADR-101 |
