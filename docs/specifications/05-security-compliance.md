...
(reached by the backend over the mesh). **A verified bearer token is required on every one of them** —
corrected 2026-09-03. ADR-019 option A had the plugin trust Kong/mesh-forwarded `x-tenant-id` /
...
| STRIDE | Threat                                                                                                                              | Mitigation                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| ------ | ----------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| S      | Forged caller on issue/verify/revoke                                                                                                | Auth plugin requires a **verified bearer** (401 `INVALID_TOKEN`); a request that resolves to no tenant identity at all is 401 `MISSING_TENANT_HEADER`. Reachable only via the internal mesh — mTLS zero-trust (§5.4, Istio); not edge-exposed for issue/revoke. Kong strips client-supplied `x-tenant-id/x-user-id/x-user-role` on the credential-service route (`request-transformer.remove`, DP-3), so a public request can never present itself as authenticated; issue/verify/revoke have no Kong route at all and are reachable only from the backend over the mesh                                                                                                                           |
...
| D      | **Flooding the two unauthenticated public GETs** — `did.json` and the Status List are reachable by anyone who knows a tenant id, and each costs a tenant-scoped database round trip | **App-layer rate limit in the service itself** — `@fastify/rate-limit`, 100 req/min (the §5.5 general limit), keyed per authenticated user and falling back to source IP, which is what the public routes resolve to. Added 2026-09-03: until then this row's mitigation was the Kong route's IP rate limit alone, and Kong is deployed nowhere (`plugins/jwt-verify.ts` records the same fact for the auth path). `/health` is registered before the limiter so probes are never throttled. Proven by `src/__tests__/rate-limit.spec.ts` |
| D      | Crypto/CPU flood on issue/verify                                                                                          | Kong `rate-limiting` (60/min, `limit_by: ip`, redis policy) on the public route; issuance is `TENANT_ADMIN`-gated and mesh-only, which limits blast radius (DP-3). The in-service limiter is the row above — this row said "No in-service limiter today" until 2026-09-03, which was true when written and false from the moment `@fastify/rate-limit` was registered; the public-GET half of the old row moved up there too |
