...
- **Vendor portal** — vendors receive RFQs, submit quotations, track PO status, submit invoices. No platform account
  required to respond to RFQs (frictionless onboarding). **Brought into MVP** implemented as a network-identity,
  two-tier (magic-link + lightweight account) external portal.
- **Contractor portal** — subcontractors track assigned tasks, submit progress updates, upload daily reports
- **Customer portal** — clients (property developers, building owners) view project progress, milestone billing status,
  QC inspection results, handover documentation

Value delivered: Remove email/phone-tag friction from external collaboration. Each portal participant added creates
a bilateral network effect — they become dependent on the platform to interact with the contractor.
