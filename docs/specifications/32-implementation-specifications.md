...
> **This table specifies payloads; it does not assert that every event is emitted.** Verified
> 2026-08-23 across all 62 committed `.avsc` schemas: **61 have a producer in application source,
> one does not** — #14 `finance.cashflow_risk.detected.v1`, marked **DECLARED** below. #12
> `finance.budget.exceeded.v1` was built the same day; the CI gate below is what noticed, failing
> with "listed in DECLARED_ONLY but now HAS a producer" before a human did.
>
> Being declared is not harmless bookkeeping. A declared event still gets a topic provisioned
> (`infrastructure/kafka/topics.yaml`), a generated TypeScript type exported from `@cos/shared`, an
...
| 14  | `finance.cashflow_risk.detected.v1`      | `project_id`, `risk_level` (enum: LOW/MEDIUM/HIGH/CRITICAL), `projected_shortfall` {amount, currency_code}, `projected_at`, `detected_by` (enum: AI_FORECAST/RULE_ENGINE) — **DECLARED; the rule is now decided, the producer is not yet built.** The forecast exists as a PULL endpoint (`GET /api/v1/finance/cashflow-forecast/:projectId`), returning 13 weekly buckets of `inflow` / `outflow` / `net_flow` / `cumulative_net`. **Decision 2026-08-23:** `RULE_ENGINE` grades by HOW SOON `cumulative_net` first goes negative — never in 13 weeks → no event; weeks 9–13 → `LOW`; 5–8 → `MEDIUM`; 2–4 → `HIGH`; 0–1 → `CRITICAL`. `projected_shortfall` is the most negative `cumulative_net` across the horizon. Everything it needs is already computed, so no new figure is invented. `AI_FORECAST` remains a second, later producer                                                                                                                                                                                                                     |
...
- **`createSerwistRoute` keeps `useNativeEsbuild` at its platform default** — `process.platform === 'win32'`.
  This rule used to say the option MUST be pinned to `false`, on the grounds that native `esbuild` "is not a
  dependency here (only `esbuild-wasm` is)". **That premise no longer holds**, so the rule was inverted rather
  than kept: `apps/web/package.json` now pins `esbuild` at the same version as `esbuild-wasm`, and
  `pnpm-workspace.yaml` carries `esbuild: true` under `allowBuilds` so its postinstall links the platform binary.
  The reversal is deliberate and its reason is recorded at `apps/web/src/app/serwist/[path]/route.ts`: the
  `esbuild-wasm` build Turbopack loads during `next build` rejects a Windows working directory, so pinning `false`
  broke the Windows build it was written to protect. The two packages are version-locked so they cannot disagree
  on the service protocol. Corrected 2026-08-22 (TDD OQ-39).
