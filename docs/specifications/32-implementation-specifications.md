...
> 2026-08-23 across all 62 committed `.avsc` schemas: **60 have a producer in application source,
> two do not** — #12 `finance.budget.exceeded.v1` and #14 `finance.cashflow_risk.detected.v1`. Both
> are marked **DECLARED** in the table below.
>
...
| 12  | `finance.budget.exceeded.v1`             | `project_id`, `cost_category`, `budget_amount` {amount, currency_code}, `actual_amount` {amount, currency_code}, `overage_percent`: DECIMAL(5,2), `detected_at` — **DECLARED, no producer.** Needs per-`cost_category` budgets, and `finance.budget_lines` has no such column. The overrun signal that IS implemented and consumed is #16 `finance.variance.alert.v1`, which is per-PROJECT against `allocated_amount` at a tenant-configurable threshold                                                                                                                                                                                                                               |
...
| 14  | `finance.cashflow_risk.detected.v1`      | `project_id`, `risk_level` (enum: LOW/MEDIUM/HIGH/CRITICAL), `projected_shortfall` {amount, currency_code}, `projected_at`, `detected_by` (enum: AI_FORECAST/RULE_ENGINE) — **DECLARED, no producer.** The forecast itself exists as a PULL endpoint (`GET /api/v1/finance/cashflow-forecast/:projectId`, 13-week direct method); what is missing is anything that grades a forecast into the four risk levels and pushes it. The thresholds and the `RULE_ENGINE` rules are unspecified — `UNSPECIFIED` per Rule 38, not a gap to fill by guessing                                                                                                                                                                                                                     |
