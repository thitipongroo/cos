...
| **Soft Delete**     | `deleted_at` is set to a timestamp on delete request — the record remains in storage. All queries filter `WHERE deleted_at IS NULL`. **Exception — File Service:** files are automatically hard-deleted from MinIO 30 days after soft delete via Temporal scheduled workflow (see `09-data-architecture` File Lifecycle Policy). Database entity records remain soft-deleted indefinitely. See `11-database-schema` section 11.4. |
