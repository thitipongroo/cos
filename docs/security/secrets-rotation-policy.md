...
| Dedicated tenant DB — RDS master (`cos_admin`)          | Cloud (AWS EKS) | AWS-managed          | `ManageMasterUserPassword=true`; AWS owns the secret and its rotation. Read-only for provisioning via `DBInstance.MasterUserSecret.SecretArn`. Never stored in `platform.tenants.dedicated_db_url`                               | AWS SM        |
| Dedicated tenant DB — `app_user` (RLS role)             | Cloud (AWS EKS) | 90 days (manual)     | Secret `cos/{env}/tenant-db/{tenantCode}/app_user` → `ALTER ROLE app_user … PASSWORD` on that tenant's DB → re-run `assignDedicatedDbActivity` so `dedicated_db_url` is rewritten (it is AES-256-GCM encrypted at rest, see F5b) | Engineering   |
...
| kg-ingestion-worker admin token (`KG_ADMIN_TOKEN`)      | All             | 90 days (manual)     | Update SM/Vault → rolling restart. Guards POST /admin/rebuild, which replays the whole topic; the worker fails closed if unset (ADR-021)                                                                                         | Engineering   |
| — (no SAST token required)                              | CI              | n/a                  | CodeQL and Semgrep CE run on the built-in `GITHUB_TOKEN`; no long-lived scanner credential exists (ADR-011)                                                                                                                      | Engineering   |
