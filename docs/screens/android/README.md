...
| [`PROJECT-MANAGER/`](PROJECT-MANAGER/)     | Tabs: **Home \| Projects \| Approvals \| Vendors** — the bar this role took on 2026-08-10 from [`mockup/mobile/06_project_manager`](../../../mockup/mobile/06_project_manager). [`01-Home/`](PROJECT-MANAGER/01-Home/) — the manager dashboard (`01`): committed spend, active projects, open issues, approvals waiting, then the project picker and the Insights panel. [`03-Approvals/`](PROJECT-MANAGER/03-Approvals/) — the PO/RFQ decision queue (`01`). [`04-Vendors/`](PROJECT-MANAGER/04-Vendors/) — the supplier directory with trust scores (`01`). `02-Projects` has no folder: that screen predates this work and is unchanged. `PROC_MANAGER` shares all three screens — its own bar is Home \| Approvals \| Vendors \| Orders — so the same script captures them with `E2E_OTP_PHONE=0811000006`. |
...
(the Privacy Policy screen, `05`) and
...
> **`PROJECT-MANAGER/` is captured by
> [`capture-android-project-manager.mjs`](../../../apps/mobile/scripts/capture-android-project-manager.mjs)**,
> which logs in as the seeded project manager (`+66811000003`) and walks Home → Approvals → Vendors in
> one run. Two things in those frames are the honest state rather than a staged one:
>
> - **Waiting on your approval reads 0 and the Approvals queue is empty.** The seed has 40 purchase
>   orders but none in `PENDING_APPROVAL` — they are all ACKNOWLEDGED or INVOICED — so there is
>   genuinely nothing to decide. Seed a PO in that state to capture the populated queue.
...
>
> The vendor cards exercise the classification added the same day: TOP RATED is DERIVED from the
> score's grade-A threshold (never stored), UNDER REVIEW is `verification_status = PENDING`, VERIFIED
> without TOP RATED is a checked vendor with no score yet, and the vendor carrying no badge at all is
> the pre-existing seed row whose two columns are still NULL.
