...
| [`PROJECT-MANAGER/`](PROJECT-MANAGER/)     | Tabs: **Home \| Procurement \| Finance \| More** — the bar from the CORRECTED [`mockup/mobile/06_project_manager`](../../../mockup/mobile/06_project_manager) set (2026-08-10). [`01-Home/`](PROJECT-MANAGER/01-Home/) — the manager dashboard (`01`): two KPI tiles, the critical-blockers card, the AI panel and YOUR PROJECTS. [`02-Procurement/`](PROJECT-MANAGER/02-Procurement/) — three counters, the AI panel and the approvals queue (`01`). [`03-Finance/`](PROJECT-MANAGER/03-Finance/) — portfolio financial summary, AI panel, per-project budget health (`01`). [`04-More/`](PROJECT-MANAGER/04-More/) — the More menu (`01`) and the vendor directory it pushes to (`02`). **`03-Approvals/` and `04-Vendors/` are STALE**: they were numbered for the earlier bar (Home \| Projects \| Approvals \| Vendors), which the product owner replaced — the Approvals screen was deleted when its queue moved into the Procurement tab, and the vendor directory moved under More. The capture script does not delete anyone's screenshots, so both folders are left for the product owner to remove. `PROC_MANAGER` no longer shares these screens: the corrected mockup set is a PROJECT_MANAGER app end to end and that role kept its original bar. |
...
> → Vendors → **Home** in one run. Several things in those frames are the honest state rather than a
> staged one:
>
