...
| Project| `DEMO-001` — *Bangkok Tower — Phase 1* (tenant `…0001`) |
...
1. **Hermes networking crash (critical).** `react-native/…/dom/events/Event.js` declares
   ...
   `Event.prototype` and throws *"Cannot assign to read-only property 'NONE'"* under Hermes —
...
2. **Analytics ignored the auth context.** The analytics controllers required `tenantId` as a
   ...
   dashboard sent neither, so it always showed *"Could not load analytics"*. Now `tenantId`
