      ...
      await syncReports([
        {
          ...
          report_date: date,
          last_known_modified_at: '2019-01-01T00:00:00.000Z',
          // A fixed date well ahead of the server row, mirroring the fixed past date the
          // older-write case below uses. `new Date()` looked equivalent and was not: the row's
          // modified_at is `now()` — the POSTGRES clock, in the container — while this is the Node
          // clock on the host, and resolveReportConflict compares the two directly with
          // `clientTs >= serverTs`. Sub-millisecond drift between the two clocks is enough to make
          // the newer write lose, which is how this case failed once in a full run and passed every
          // time it was run alone. The product comparing a device clock to the server's is the
          // documented LAST_WRITE_WINS design; a TEST resting on those two clocks agreeing is not.
          client_submitted_at: '2099-01-01T00:00:00.000Z',
        },
      ]);
