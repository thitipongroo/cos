...
describe('Phase 14 · Kafka → ClickHouse → API (master:4330)', () => {
  ...
  beforeAll(async () => {
    ...
    const { KafkaTopicProvisioner } = await import('@cos/shared');
    ...
    const workerBin = path.join(os.tmpdir(), 'cos-analytics-worker-under-test');
  ...
  it('ingests a published event into the aggregate table', async () => {
    ...
    // (test/helpers/integration-mocks.ts) replaces @cos/shared's KafkaProducer with a no-op,
    ...
    const { KafkaProducer } = jest.requireActual<typeof import('@cos/shared')>('@cos/shared');
