...
describe('BoqService', () => {
  ...
  describe('version total sums ROOT categories only (master:2292)', () => {
    ...
    it('excludes a child category from the version total', async () => {
      ...
      expect(mockRepo.updateVersionTotal).toHaveBeenCalledWith('version-uuid-001', '100.0000');
    });
    ...
    it('sums every root category when there is more than one', async () => {
      ...
      expect(mockRepo.updateVersionTotal).toHaveBeenCalledWith('version-uuid-001', '107.5000');
    });
  });
