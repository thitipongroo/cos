// FROZEN COPY — do not edit. This is `enterprise-provisioning.workflow.ts` exactly as it stood on
// 2026-09-14, BEFORE its query was renamed from 'state' to 'workflowState' (§34.3). It exists for one
// test, `enterprise-provisioning-query-rename.workflow.spec.ts`, which starts a workflow on THIS code,
// stops that worker, and resumes the same execution on the renamed code — the situation a worker
// deploy creates for a provisioning run parked at AWAITING_APPROVAL, which waits indefinitely.
// Only the activities import path differs from the original, because this file lives in fixtures/.
...
} from '../../workflows/enterprise-provisioning.activities';
