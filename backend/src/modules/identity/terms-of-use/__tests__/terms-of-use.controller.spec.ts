// Terms of Use document controller tests (ADR-092).
//
...
const PDF = {
  ...
  sha256: 'b'.repeat(64),
  fileName: 'COS_Terms_of_Use_v1.0.0.pdf',
  version: '1.0.0',
...
describe('TermsOfUseController.metadata', () => {
  it('returns what the client needs to verify the download', async () => {
    const controller = new TermsOfUseController(service as never);
  ...
  it('states the language, so a Thai reader is not left guessing', async () => {
    ...
    const { language } = await new TermsOfUseController(service as never).metadata();
    expect(language).toBe('en');
  });
});

describe('TermsOfUseController.pdf', () => {
  it('returns the bytes with the download headers', async () => {
    const { reply, headers } = replyDouble();

    const body = await new TermsOfUseController(service as never).pdf(reply as never);

    expect(headers['Content-Type']).toBe('application/pdf');
    expect(headers['Content-Disposition']).toBe(
      'attachment; filename="COS_Terms_of_Use_v1.0.0.pdf"',
    );
  ...
  it('uses the content digest as the ETag', async () => {
    ...
    await new TermsOfUseController(service as never).pdf(reply as never);
