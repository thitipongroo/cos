...
Validates all required environment variables at startup with a clear error message listing every missing/invalid variable. Prevents services from starting with incomplete configuration. All secrets are injected via AWS Secrets Manager (cloud) or HashiCorp Vault (on-premise) — never hardcoded (QM-4).
...
Parses and validates `process.env` against the Zod schema. Throws with a detailed error if validation fails. Call once at module startup.
...
- Additional service-specific variables (e.g. `KEYCLOAK_BASE_URL`) are validated in the respective module's own config schema, extending `AppConfig`
