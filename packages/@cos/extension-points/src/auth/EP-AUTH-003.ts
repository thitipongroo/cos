...
export class EnterpriseSSOProvider extends StubBase {
  ...
  readonly TRIGGER = 'Enterprise customer has existing IdP (AD/Okta) requiring SAML 2.0 federation into Keycloak realm';
  ...
  async configureSamlIdp(tenantId: string, idpMetadataUrl: string): Promise<void> {
