...
- **Authentication — TWO PATHS (Phase 2 authoritative):**
  ...
  - **Who may use which path — spec §5.4.4 (PO decision 2026-08-21):** both paths are open to every role EXCEPT `TENANT_ADMIN` and `FINANCE`, which are **Path B only**. Path A mints its token through Direct Grant, which Keycloak binds separately from the browser flow, so the ADR-067 MFA condition cannot run there — the Direct Grant flow denies those two roles at the IdP; and NIST SP 800-63B Rev 4 makes SMS a restricted authenticator that no longer satisfies AAL2. A path also requires the matching identifier on the account (phone for A, email + Keycloak credential for B), and `POST /api/v1/users` provisions one or the other
