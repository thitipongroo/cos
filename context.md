...
- SAST and code quality scan must pass in CI before merge — **CodeQL + Semgrep CE + jscpd** (ADR-070;
  ...
  - **ruff** (`ruff check services mlops`, CI lint job, BLOCKING) — Python lint. Nothing linted
    ...
    was measured against this tree and passes clean. Runs offline (ADR-070).
  ...
  - Coverage thresholds are enforced where they are measured — jest 100/100 (QM-1) and pytest
    ...
    Developer Edition (paid). See ADR-070 for the full comparison and the air-gapped caveat.
...
# 4. SAST + code quality scan (ADR-070 — replaced SonarQube)
