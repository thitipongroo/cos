// System-integration screenshot capture — adb/uiautomator only. Logs in as TENANT-ADMIN, opens Quick
...
//   docs/screens/android/04-tenant-admin/01-Home/07-ta-system-integration.png
...
const OUT_DIR = resolve(HERE, '../../../docs/screens/android/04-tenant-admin/01-Home');
const OUT = join(OUT_DIR, '07-ta-system-integration.png');
...
async function main() {
  ...
  for (let i = 0; i < 8; i++) {
    ...
    console.log(`  shot ${i}`);
    if (i < 7) {
      adb('shell', 'input', 'swipe', '540', '1900', '540', '1200', '450');
      await delay(1400);
    }
  }
