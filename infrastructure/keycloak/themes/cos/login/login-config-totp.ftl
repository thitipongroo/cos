<#--
  ...
  custom login.ftl) styled to match mockup/mobile/04_tenant_admin/03_mfa_enrollment_mobile_view_restored
  for the parts Keycloak owns (PO decision: theme the Keycloak-owned parts exactly, omit the mockup's
  backup-codes bento / audit-trail / bottom-nav — those belong to the 06 recovery page, app/backend data,
