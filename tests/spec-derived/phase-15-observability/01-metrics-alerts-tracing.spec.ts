...
describe('Phase 15 · trace propagation (master:4399)', () => {
  it('propagates W3C TraceContext across Kafka', () => {
    // HTTP propagation comes free with the SDK; Kafka does not — an async hop loses the trace
    // unless the headers are carried deliberately.
    const kafka = read('packages/@cos/tracing/src/kafka-propagation.ts');
    expect(kafka).toMatch(/traceparent/);
    expect(kafka).toMatch(/tracestate/);
  });
});
...
describe('Phase 15 · collection and retention (master:4426-4439)', () => {
  ...
  it('the NestJS interceptor records HTTP metrics automatically', () => {
    // "automatic" is the point: a per-controller call would miss whichever route someone forgot.
    expect(exists('backend/src/shared/interceptors/http-metrics.interceptor.ts')).toBe(true);
    expect(read('backend/src/app.module.ts')).toMatch(
      /APP_INTERCEPTOR,\s*useClass:\s*HttpMetricsInterceptor/,
    );
  });

  it('requests that reach no route are counted too (master:4357)', () => {
    // An interceptor runs inside the route pipeline and cannot see them, so http_requests_total
    // used to omit every 404 on an unknown path — the shape a scanner or a wrong-prefix client
    // makes. Middleware covers the gap; the shared marker stops the two double-counting.
    expect(exists('backend/src/shared/middleware/unmatched-route-metrics.middleware.ts')).toBe(
      true,
    );
    expect(read('backend/src/app.module.ts')).toMatch(/UnmatchedRouteMetricsMiddleware/);
    const shared = read('backend/src/shared/interceptors/http-metrics.shared.ts');
    expect(shared).toMatch(/METRICS_RECORDED/);
    // One instrument pair between them: two createMetrics() calls would register duplicate series
    // under the same name.
    expect(shared).toMatch(/export const httpMetrics = createMetrics\(\)/);
  });

  it('an unmatched path does not become its own metric series', () => {
    // The URL is client-controlled; per-path series on 404s is a cardinality explosion.
    expect(read('backend/src/shared/middleware/unmatched-route-metrics.middleware.ts')).toMatch(
