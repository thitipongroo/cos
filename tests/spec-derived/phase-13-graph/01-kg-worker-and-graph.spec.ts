...
describe('Phase 13 · the Delay node has no source yet — deferred to Phase 23', () => {
  ...
  it('the event is still declared, so the contract survives the wait', () => {
    ...
    expect(exists('packages/@cos/shared/src/avro/construction.delay.detected.v1.avsc')).toBe(true);
    expect(read('packages/@cos/shared/src/kafka/topic-catalog.ts')).toMatch(
