...
describe('Phase 19 · the verification script (master:4877)', () => {
  ...
  it('is executable', () => {
    // Asked of GIT, not of the filesystem. The mode that matters is the one recorded in the tree —
    // that is what CI checks out and what ships. NTFS cannot represent an exec bit at all, so
    // fs.statSync returns 0o666 for every file on a Windows clone and this case failed there while
    // the committed mode was 100755 the whole time. Reading the index is both portable and the
    // stricter question: a mode fixed only in a working copy is a mode that never reaches anyone.
    const entry = execFileSync('git', ['ls-files', '-s', '--', verifyScript], {
      cwd: repoRoot,
      encoding: 'utf8',
    });
    const mode = /^(\d{6}) /.exec(entry)?.[1];
