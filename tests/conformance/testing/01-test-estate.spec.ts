...
describe('Phase 18 · k6 load scenarios (master:4776-4791)', () => {
  ...
  it('CI runs all four scenarios, not one of them — plus the QM-6 gate', () => {
    ...
    // not a test.
    //
    // qm6-baseline is the fifth entry and not a Phase 18 scenario — it is the QM-6 sustained-load
    // gate §30.9 also schedules weekly. It is asserted here rather than left out because being in
    // the loop is the whole difference between a gate and a file (§35.13 ESC-12).
  ...
  it('the QM-6 gate asserts the §31.6 budgets, not the Phase 18 one', () => {
    // The two scripts are one letter apart in purpose and were confused once already: the QM-6 gate
    // was deleted on 2026-08-29 as a duplicate of tests/load/api-baseline.js, which shares neither
    // its load profile nor its thresholds. Pinning the numbers is what makes them distinguishable
    // to something other than a careful reader.
    const qm6 = read('tests/load/qm6-baseline.js');
    expect(qm6).toMatch(/vus:\s*100\b/);
    expect(qm6).toMatch(/duration:\s*'5m'/);
    expect(qm6).toMatch(/'http_req_duration\{op:read\}':\s*\['p\(95\)<300'\]/);
    expect(qm6).toMatch(/'http_req_duration\{op:write\}':\s*\['p\(95\)<500'\]/);
    expect(qm6).toMatch(/http_req_failed:\s*\['rate<0\.001'\]/);
  });
