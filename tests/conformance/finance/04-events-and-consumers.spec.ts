...
describe('Phase 7 · every emitted event is one the platform can carry', () => {
  const catalogue = read('packages/@cos/kafka/src/topic-catalog.ts');
  ...
  it('the variance alert names the same event everywhere it is written down', () => {
    ...
    // outboxEvent, not emitEvent: §35.13 ESC-13 moved every finance emission onto the transactional
    // outbox, so the service builds an envelope the repository write anchors instead of publishing
    // to Kafka inline. The event NAME is what this case is about, and it is unchanged.
    expect(service).toContain(`outboxEvent('${name}'`);
    expect(catalogue).toContain(`'${name}': '${name}.avsc'`);
    expect(exists(`packages/@cos/kafka/src/avro/${name}.avsc`)).toBe(true);
