...
import { exists, read, readYaml, repoRoot, toPosix } from '../helpers';
  ...
  const walk = (dir: string): void => {
    for (const e of fs.readdirSync(dir, { withFileTypes: true })) {
      ...
      if (e.isDirectory()) {
        ...
        out.push([toPosix(path.relative(repoRoot, full)), fs.readFileSync(full, 'utf8')]);
      }
    }
  };
...
describe('Phase 16 · RLS is the primary isolation (master:4493-4497)', () => {
  ...
  // migration. backend/test/security/01-tenant-isolation-and-audit.integration now proves
...
describe('Phase 16 · rate limits (master:4486-4488, 4514-4519)', () => {
  ...
  // backend/test/security/02-headers-and-rate-limits.integration: the eleventh auth
...
describe('Phase 16 · secure headers (master:4535-4541)', () => {
  ...
  // backend/test/security/02-headers-and-rate-limits.integration, together with the
