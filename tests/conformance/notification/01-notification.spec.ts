 ...
 * backend/test/notification/01-delivery.integration — including the platform
...
import { read, readYaml, abs, toPosix } from '../helpers';
...
describe('no service sends notifications directly (master:5041)', () => {
  it('confines the notification ADAPTERS to one documented exception', () => {
    ...
    const ALLOWED = ['identity/subject-request/subject-request.service.ts'];
    const offenders = backendSources
      ...
      .map((f) => toPosix(path.relative(abs('backend/src/modules'), f)))
...
describe('every routed event is also subscribed (master:5118)', () => {
  ...
  it('subscribes by RegExp so a tenant provisioned later is still heard', () => {
    ...
    const kafkaConsumer = read('packages/@cos/kafka/src/consumer.ts');
