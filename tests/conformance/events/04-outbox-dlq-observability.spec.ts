...
const outboxWriter = read('packages/@cos/kafka/src/outbox.ts');
...
// unchecked. See tests/conformance/events/06-rule-34.spec.ts.
...
const dlq = read('packages/@cos/kafka/src/dlq.ts');
const consumer = read('packages/@cos/kafka/src/consumer.ts');
const metrics = read('packages/@cos/kafka/src/metrics.ts');
...
describe('Phase 8 · OpenTelemetry propagation (master:3166)', () => {
  it('the producer writes W3C traceparent into the Kafka headers', () => {
    ...
    expect(read('packages/@cos/kafka/src/producer.ts')).toContain('traceparent');
  });
});
