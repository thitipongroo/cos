 ...
 * in the Kafka SDK's outbox.ts and exported from its index.ts until 2026-08-27, duplicating
...
describe('Rule 34(c) — no Node-runtime polling loop inside @cos/shared', () => {
  ...
  it('keeps OutboxPublisher, which is the half that belongs in a package', () => {
    ...
    //
    // It is asserted against \bcos/kafka, not \bcos/shared: ADR-055 moved the whole Kafka runtime into
    // the server-only package on 2026-08-22, leaving \bcos/shared holding event TYPES alone. The
    // publisher had to land somewhere, and a package is still the right place for it.
    expect(fs.readFileSync(abs('packages/@cos/kafka/src/index.ts'), 'utf8')).toMatch(
      /\bOutboxPublisher\b/,
    );
  });

  it('does not put the poller in @cos/kafka either — it is not a package problem', () => {
    // The class moved out of @cos/shared with everything else. Rule 34(c) says backend/src/, and
    // that is where the surviving one is; re-exporting it from the server-only package would satisfy
    // the letter of clause (c) while recreating the duplicate it exists to prevent.
    expect(fs.readFileSync(abs('packages/@cos/kafka/src/index.ts'), 'utf8')).not.toMatch(
      /\bOutboxPoller\b/,
    );
    expect(fs.existsSync(abs('backend/src/shared/events/outbox-poller.service.ts'))).toBe(true);
  });
...
describe('Rule 34 — the client-safe packages stay importable from a mobile bundle', () => {
  ...
  const NODE_ONLY = [
    'shared',
    // ADR-055 (2026-08-22) moved the Kafka SDK out of @cos/shared into its own package: kafkajs,
    // ioredis and prom-client are runtime Node dependencies, so the package that carries them must
    // never be reachable from a bundle. It is node-only for exactly the reason this list exists.
    'kafka',
    'database',
    'logger',
    'tracing',
