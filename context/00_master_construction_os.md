...
<!-- v1.17.0 (2026-05-31): Phase 8 Generate — integration tests made explicit (prevent new-project recurrence):
     Bug-6 FIX: Phase 8 Generate — "Integration tests" → explicit file path + library + scripts:
       File:    packages/@cos/shared/test/kafka/kafka.integration.spec.ts
       Deps:    add testcontainers ^10.9.0 + @testcontainers/kafka ^10.9.0 to @cos/shared devDependencies (Rule 27)
       Script:  add test:integration (jest --testPathPattern='test/') to @cos/shared package.json (Rule 28)
                turbo.json already has test:integration task — no change needed there
       Approach: KafkaContainer (real broker) + mock Schema Registry + mock Redis
       Cases:   (a) producer publishes → consumer receives correct payload
                (b) same event_id processed exactly once (idempotency)
     Overall system grade: A (0 Critical, 0 Major, 0 Minor) -->
<!-- v1.16.0 (2026-05-31): Commands made specific enough to prevent bug recurrence in new projects:
     Bug-2 FIX: Phase 2 Generate — "OpenAPI 3.1 spec" → explicit file paths:
       docs/api/auth.openapi.yaml + docs/api/tenant.openapi.yaml (two separate files)
     Bug-3 FIX: Phase 2 Generate — "MFA endpoints" → explicit paths:
       POST /api/v1/auth/mfa/enroll, /mfa/verify, /mfa/authenticate
     Bug-4 FIX: Phase 8 Generate — "Kubernetes manifests" → explicit path + filenames:
       infrastructure/kubernetes/kafka/kafka-statefulset.yaml
       infrastructure/kubernetes/kafka/schema-registry-deployment.yaml
     Bug-5 FIX: Phase 1 Generate — "README per service" → explicit list of ALL directories:
       services/, apps/, backend/, backend/src/modules/*, packages/@cos/* (all 11)
     Overall system grade: A (0 Critical, 0 Major, 0 Minor) -->
<!-- v1.15.0 (2026-05-31): Rule 37 added to ALL 24 phases — Constraints section updated:
     Phase 1, 2, 7, 8, 12 — Rule 37 appended to existing Constraints
     Phase 3–6, 9–11, 13–23 — new Constraints section created with Rule 37
     Phase 24 — Rule 37 appended to existing Constraints
     Total: 24/24 phases now have mandatory exhaustive verification before marking complete -->
<!-- v1.14.0 (2026-05-31): Rule 37 added to master spec — exhaustive verification mandatory:
     Rule 37 added to: Phase 1 Constraints, Phase 2 Constraints, Phase 8 Constraints (new section),
       ROOT CAUSE PREVENTION RULES (after Rule 36)
     Root cause: agent was reporting "complete" based on partial checks of known bugs only,
       not reading every Generate item line by line against filesystem
     Now every Phase has explicit Constraint: "read every Generate item, run ls/grep, show output"
     Overall system grade: A (0 Critical, 0 Major, 0 Minor) -->
<!-- v1.13.0 (2026-05-31): 4 long-term vulnerabilities fixed:
     V-01 RESOLVED: context.md + context/ removed from .gitignore — now tracked by git;
       Rules 27-36 persist across git clone; no longer machine-local only
     V-02 RESOLVED: Phase 2 Generate — npm packages list added (Rule 27):
     ...
     V-03 RESOLVED: Phase 2 Generate — ADR-008 added as explicit deliverable (Rule 30):
       "Create docs/architecture/adr/008-*.md BEFORE implementing TenantPrismaService"
     V-04 RESOLVED: Phase 1 Generate — jest.config packages list expanded from 3 → 9 (Rule 36):
       Added @cos/financial, @cos/rbac, @cos/validation, @cos/logger, @cos/tracing, @cos/config
     Overall system grade: A (0 Critical, 0 Major, 0 Minor) -->
<!-- v1.12.0 (2026-05-31): Root cause prevention rules completed — 4 additional rules (33–36):
     Rule 33: Single source of truth for jest config — no duplicate jest key in package.json when jest.config.js exists
     Rule 34: import type for non-runtime deps — prevents mobile/Metro bundling Node.js-only packages
     Rule 35: @cos/shared must be framework-agnostic — no server-only runtime imports
     Rule 36: All @cos packages with executable logic must have unit tests + jest config + CI coverage
     Overall system grade: A (0 Critical, 0 Major, 0 Minor) -->
<!-- v1.11.0 (2026-05-31): Root cause prevention rules added — 6 new rules (27–32):
     Rule 27: Package dependency sync — verify package.json before adding imports
     Rule 28: turbo.json sync — update when adding new scripts to any package.json
     Rule 29: pnpm lock file — run pnpm install + commit pnpm-lock.yaml after package.json changes
     Rule 30: ADR reference verification — verify ADR file exists before writing (see ADR-NNN)
     Rule 31: Async fake timer pattern — jest.runAllTimersAsync() for async retry/sleep chains
     Rule 32: Generate section completeness — exhaustive scope for README/Tooling/tsconfig exceptions
     Phase 1 Generate updated: pnpm-lock.yaml step added (Rule 29)
     Phase 18 Generate updated: async fake timer pattern added (Rule 31)
     Overall system grade: A (0 Critical, 0 Major, 0 Minor) -->
<!-- v1.10.0 (2026-05-31): Test infrastructure — Phase 1 deliverable (7 Decisions resolved 2026-05-31):
     M-01 RESOLVED: jest.config added as Phase 1 Generate deliverable (Decision 2);
       coverage thresholds {lines:80, branches:70} per QM-1; collectCoverageFrom excludes
       non-executable files; moduleNameMapper for @cos/* workspace paths;
       packages: backend/, @cos/shared, @cos/database
     Overall system grade: A (0 Critical, 0 Major, 0 Minor) -->
<!-- v1.9.0 (2026-05-31): Phase 1 spec gap resolution — 4 fixes applied:
     M-01 RESOLVED: Phase 1 Generate — added Husky init step (initialize husky, create pre-commit hook running lint-staged);
     M-02 RESOLVED: Phase 1 Generate — added mobile tsconfig exception (React Native requires expo/tsconfig.base; mobile-compatible @cos/* paths listed);
     M-03 RESOLVED: Phase 1 directory @cos/shared — clarified Avro schemas are Phase 8 deliverable, not Phase 1;
     M-04 RESOLVED: Phase 1 directory @cos/database — corrected ADR reference from ADR-008 to ADR-015 (ADR-008 is TenantPrismaService; ADR-015 is retry helpers);
     Overall: Phase 1 spec now fully specifies all deliverables without ambiguity -->
<!-- v1.8.0 (2026-05-29): cos-context-auditor round 6 — 1 fix applied:
     M-01 RESOLVED: Security Requirements — added AES-256 minimum encryption algorithm rule (spec §5.2);
     Overall system grade: A (0 Critical, 0 Major, 0 Minor) -->
<!-- v1.7.0 (2026-05-29): cos-context-auditor round 5 — 4 fixes applied:
     M-01 RESOLVED: FINAL EXECUTION ORDER — Event Infrastructure dep corrected Phase 1 → Phase 2 (spec §32.1);
     M-02 RESOLVED: WORKFLOW ENGINE SPEC RFQ — EVALUATED→AWARDED and EVALUATED→CANCELLED
       changed from PROC_MANAGER → PROCUREMENT_OFFICER (spec §32.6 + §6.2);
     m-01 RESOLVED: master rule 25 duplicate — second rule 25 renumbered to rule 26;
     Overall system grade: A (0 Critical, 0 Major, 0 Minor) -->
<!-- v1.6.0 (2026-05-29): cos-context-auditor round 4 — 4 fixes applied:
     M-01 RESOLVED: §6 BASE EVENT ENVELOPE — added event_version field (spec §32.4);
     m-NEW-1 RESOLVED: §6 migration note updated — Group A MIGRATED (16 files deleted 2026-05-29);
     m-01 RESOLVED: Stage 1 name "Single Tenant MVP" → "Multi-tenant MVP" (spec §32.1);
     m-02 RESOLVED: EP stub example (CarbonCalculationEngine) now includes EP_VERSION field -->
<!-- v1.5.1 (2026-05-27): cos-context-auditor round 3 — 1 Major fix:
     M-01 RESOLVED: RBAC Role Definitions table — 3 spec §6.2 roles were missing:
       EXECUTIVE (used in workflow approval rules lines ~1046/1059/1060 — now defined),
       SAFETY_OFFICER (used in safety permit workflow line ~1061 — now defined),
       CRM_SALES_MANAGER (no prior identifier — now defined per spec §6.2);
       Implementation sub-roles (PROC_MANAGER, SITE_WORKER, VIEWER) retained but
       explicitly labeled as not-in-spec sub-roles for implementation granularity -->
<!-- v1.5 (2026-05-27): PO decisions applied — final 2 open items resolved:
     C-02 RESOLVED: Secrets management — conditional per deployment type (spec §5.2; ADR-013);
       Infrastructure stack: Vault unconditional → AWS SM (cloud/EKS) + Vault (on-prem/hybrid);
       Phase 2 Secret Management section rewritten for conditional strategy;
       Phase 19 Secret Management section updated (cloud: ESO; on-prem: Vault sidecar);
       SecretRotation note updated: cloud uses AWS SM rotation; on-prem uses Vault DB engine;
     C-03 RESOLVED: RBAC role identifiers aligned to spec §6.2 (ADR-014);
       SITE_MANAGER → SITE_ENGINEER (9 occurrences);
       PROC_OFFICER → PROCUREMENT_OFFICER (6 occurrences);
       FINANCE_OFFICER → FINANCE (13 occurrences);
       SUPER_ADMIN → SYSTEM_ADMIN (1 occurrence);
       RBAC Role Definitions table reformatted and aligned to spec §6.2;
     ALL OPEN ITEMS RESOLVED — no remaining AWAITING_DECISION flags -->
<!-- v1.4 (2026-05-27): PO decisions applied — 3 open items resolved:
     C-01 RESOLVED: Kong Gateway for rate limiting (spec §4.8; ADR-010);
       API Gateway section updated: NestJS ThrottlerModule → Kong Gateway;
       API monetization scope narrowed to billing/metering only (Kong is the infrastructure);
     C-04 RESOLVED: SonarQube for SAST + code quality (spec §30.10, §30.12; ADR-011);
       (Phase 19 SAST check updated in context.md; master had no semgrep reference);
     C-05 RESOLVED: ArgoCD for GitOps CD (spec §4.9; ADR-012);
       Phase 17 pipeline updated: GitHub Actions CI-only + ArgoCD CD (no more kubectl in CI);
       Phase 19 CI/CD checks updated to ArgoCD commands;
       Infrastructure stack: ArgoCD AWAITING_DECISION → RESOLVED;
     OPEN (unchanged): C-02 Vault vs AWS SM; C-03 RBAC role names -->
<!-- v1.3 (2026-05-27): cos-context-auditor re-audit (round 2) — fixes applied:
     C-NEW-2: 5 canonical event names corrected in §6 CROSS-SERVICE EVENT CONTRACT (spec §32.4):
       Event 5: construction.site_report.created.v1 → site.report.created.v1
       Event 6: construction.inspection.failed.v1 → site.inspection.failed.v1
       Event 9: workforce.attendance.checkin.created.v1 → workforce.checkin.created.v1
       Event 10: construction.material.consumed.v1 → site.material.consumed.v1
       Event 13: finance.vendor_invoice.approved.v1 → procurement.vendor_invoice.approved.v1;
     M-NEW-1: Phase 20 in-app channel WebSocket → SSE (spec §19.2 explicitly prohibits WebSocket);
              Phase 20 push channel FCM only → Expo Push Notifications (APNs + FCM) via expo-server-sdk;
     M-NEW-2: Phase 20 trigger event construction.inspection.failed.v1 → site.inspection.failed.v1 (spec §32.4 #6);
              Phase 20 trigger event construction.site_report.submitted.v1 → site.report.created.v1 (spec §32.4 #5);
     M-NEW-3: EmailProvider annotated as PARTIAL-RESOLUTION (spec §19.7: SendGrid MVP → AWS SES production);
     m-NEW-1: Blockquote header Version/Last updated corrected (1.0/2026-05-26 → 1.3/2026-05-27) -->
<!-- v1.2 (2026-05-27): cos-context-auditor full audit — fixes applied:
     C-NEW-1: Kafka compatibility BACKWARD → BACKWARD_TRANSITIVE (§6, §Phase 8, Rule 24, Phase 19 AUTO check; spec §32.4);
     M-1: coverage 80% lines → 80% lines + 70% branches (Rule 11, Phase 18, Phase 8; spec §30.3);
     M-5: CROSS-SERVICE EVENT CONTRACT — all 15 events annotated with canonical {domain}.{entity}.{action}.v{N} names; event_type envelope updated; Phase 20 notification triggers updated (spec §32.4, §15.6);
     M-4: Apache Iceberg added to GLOBAL SYSTEM CONTEXT COMMAND infrastructure stack + Phase 17 cold storage (spec §4.3, §9.3);
     M-6: Debezium CDC Path 2 added to infrastructure stack + Phase 8 stub DebeziumCDCPipeline (spec §4.4, §9.4);
     M-12: PO approval thresholds added to WORKFLOW ENGINE SPEC — ≤50K/50K-500K/>500K THB chains (spec §15.5);
     m-2: Phase 15 sampling rate 10% → 1% production (spec §31.5);
     FLAGGED C-04 AWAITING_DECISION: SonarQube vs semgrep — ADR required;
     FLAGGED C-05 AWAITING_DECISION: ArgoCD vs GitHub Actions+kubectl — ADR required;
     OPEN (requires PO): C-01 Kong vs NestJS rate limiting; C-02 Vault vs AWS SM;
     C-03 RBAC role names spec §06 vs master -->
<!-- v1.1 (2026-05-26): cos-context-auditor C-05 fix — all /v1/ API paths updated to /api/v1/ in
     Phases 3, 4, 6, 7, 11, 12, 13, 14, 15, 18, 21 (32 endpoints corrected);
     M-01 Phase 19 runbook path corrected (docs/runbooks/ → docs/03-runbooks/);
     M-02 Phase 19 architecture path corrected (docs/architecture/ → docs/01-architecture/);
     m-01 header last_updated date corrected (2026-05-25 → 2026-05-26);
     m-02 global arch rule updated (/v1/ → /api/v1/);
     OPEN (requires PO): C-01 Kong vs NestJS rate limiting; C-02 Vault vs AWS SM;
     C-03 RBAC role names spec §06 vs master -->
...
- CloudflareWAFIntegration (ep-waf-001.ts): update from UNSPECIFIED stub → concrete class
