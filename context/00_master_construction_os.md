...
- Playwright E2E tests (web — location: tests/e2e/; runs on merge to `staging` (ADR-068); source: spec §30.5 + Phase 18 Generate):
...
- Detox E2E tests (React Native mobile — location: apps/mobile/e2e/; runs on merge to `staging` (ADR-068); source: spec §30.5, §30.7):
