...
metric is a permanently-down target, indistinguishable from a real outage. This is the one
implementation of that endpoint, re-exported by each service's ``metrics.py``; the Go workers have
the equivalent in ``libs/go/cosmetrics``. It replaces four byte-identical copies that jscpd flagged.
