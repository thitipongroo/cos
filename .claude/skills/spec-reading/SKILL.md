...
In this repository the tags are `READY` and `NEEDS_ESCALATION`, and an escalation must meet one of two definitions:
`UNSPECIFIED` (absent from all spec and context files) or `AWAITING_DECISION` (a pending decision marker in context
files 05-11). Credentials, resolved technologies, and anything the spec marks RESOLVED do not qualify. Rule 38 also
forbids delegating the reading to a subagent.
