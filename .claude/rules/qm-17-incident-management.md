...
- **On-call rotation** — defined in `docs/runbooks/on-call-rotation.md`; on-call engineer must have live access to
  Grafana, Alertmanager/Prometheus, Temporal console, and Kubernetes before going on-call
...
- **Status page** — required before Stage 2 go-live; auto-updates from Alertmanager/Prometheus alerts; managed via
...
- **Post-mortem** — blameless; must include: root cause, timeline, impact assessment, action items with owners and due
  dates; template in `docs/runbooks/postmortem-template.md`
